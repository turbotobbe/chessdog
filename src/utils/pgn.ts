...
export function parsePgn(content: string): PgnMove[] {
    console.log(content)
    const moves: PgnMove[] = [];

    // Split the content on empty lines and get the last section (moves)
    const sections = content.split(/\n\s*\n/);
    const moveSection = sections[sections.length - 1].trim();

    // Split the moves on valid move numbers (1. or 1...)
    const steps = moveSection.split(/\d+\.{1,3} /).filter(Boolean);
    ...
    for (const currentStep of steps) {
        ...
        const [_, stepStr, clkStr] = currentStep.trim().match(/(.*)\s+\{\[%clk\s+(.+)\]\}/) || [];
        ...
        if (isWhite) {
            moves.push({ white: step, black: {move: '', clk: ''} });
        } else {
            moves[moves.length - 1].black = step;
        }
        isWhite = !isWhite;
    }
    console.log(moves);
